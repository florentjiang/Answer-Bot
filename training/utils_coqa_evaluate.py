...
def parse_args():
  parser = argparse.ArgumentParser('Evaluation script for CoQA')
...
def make_qid_to_has_ans(dataset):
  qid_to_has_ans = {}
  for p in dataset:
      for ans,qa in zip(p['questions'],p["answers"]):
        qid = qa["turn_id"]
        aid = ans["turn_id"]
        qas_id = p["id"]+str(qid)
        ans_id = p["id"]+str(aid)
        ...
        qid_to_has_ans[p["id"]+str(qid)] = True
...
def get_raw_scores(dataset, preds):
  ...
  f1_scores = {}
  for p in dataset:
      for qa,ans in zip(p['questions'],p['answers']):
        q_id = qa["turn_id"]
        a_id = ans["turn_id"]
        if q_id != a_id:
            print('error '+q_id+' : '+a_id)
        qid = p["id"]+str(q_id)
        answer = ans["span_text"]
        if "best_f1_answer_start" in ans:
            answer = ans["best_f1_answer"]
