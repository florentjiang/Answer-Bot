...
from utils_newsqa_evaluate import find_all_best_thresh_v2, make_qid_to_has_ans, get_raw_scores
...
class NewsqaExample(object):
    """
    A single training/test example for the Newsqa dataset.
...
def read_coqa_examples(input_file, is_training, version_2_with_negative, group):
    """Read a SQuAD json file into a list of NewsqaExample."""
    with open(input_file, "r", encoding='utf-8') as reader:
        input_data = json.load(reader)["data"]
    ...
    for paragraph in input_data:
            ...
            for c in paragraph_text:
                if is_whitespace(c):
                    ...
                    prev_is_whitespace = False
                char_to_word_offset.append(len(doc_tokens) - 1)
            for qa,ans in zip(paragraph["questions"],paragraph["answers"]):
                qid = qa["turn_id"]
                aid = ans["turn_id"]
                qas_id = paragraph["id"]+str(qid)#qa["id"]
                ans_id = paragraph["id"]+str(aid)
                if qas_id != ans_id:
                    print('error '+qas_id+' : '+ans_id)
                question_text = qa["input_text"]
                start_position = None
                end_position = None
                orig_answer_text = None
                is_impossible=False
                if is_training:
                    if not is_impossible:
                        ...
                        answer_end_offset = ans["span_end"]
                        start_position = char_to_word_offset[answer_offset]
                        end_position = char_to_word_offset[answer_end_offset-1]
                        answer_length = answer_end_offset-answer_offset
                        orig_answer_text = ans["span_text"]
                    if "best_f1_answer_start" in ans:
                        orig_answer_text = ans["best_f1_answer"]
                        start_position = answer["best_f1_answer_start"]
                        end_position = answer["best_f1_answer_end"]
                        # Only add answers where the text can be exactly recovered from the
                        # document. If this CAN'T happen it's likely due to weird Unicode
                        # stuff so we will just skip the example.
                        #
...
def write_predictions_extended(all_examples, all_features, all_results, n_best_size,
                                ...
                                tokenizer, verbose_logging, modelname):
    """ XLNet write prediction logic (more complex than Bert's).
        Write final predictions to the json file and log-odds of null if needed.

        Requires utils_newsqa_evaluate.py
    """
    _PrelimPrediction = collections.namedtuple(  # pylint: disable=invalid-name
        "PrelimPrediction",
