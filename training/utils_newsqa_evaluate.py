...
def parse_args():
  parser = argparse.ArgumentParser('Evaluation script for NewsQA')
...
def make_qid_to_has_ans(dataset):
  ...
  for p in dataset:
    if p['type']=="dev":
      for count, qa in enumerate(p['questions']):
        if 'consensus' not in qa:
            qid_to_has_ans[p["storyId"]+str(count)]=False
        elif 's' not in qa["consensus"] or 'e' not in qa["consensus"]:
            qid_to_has_ans[p["storyId"]+str(count)]=False
        else:
            qid_to_has_ans[p["storyId"]+str(count)] = True
    ...
    if p['type']=="dev":
      for count, qa in enumerate(p['questions']):
        ...
        if 'consensus' not in qa:
            ...
            is_impossible=True
        if 'noAnswer' in qa["consensus"]:
            if qa["consensus"]["noAnswer"]:
                is_impossible=1.0
        if not is_impossible:
            answer_offset = answer["s"]
            answer_end_offset = answer["e"]
            orig_answer_text = p['text'][answer_offset:answer_end_offset-1]
